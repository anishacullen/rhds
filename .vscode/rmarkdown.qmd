...
```{r} [Runs the glm testing for an association between protein levels and tissue type]
...
```{r}[Plots figures to visualise the results]
fit.coefs |>
...
```{r}[Runs the glm testing for associations between protein levels and progression free interval]
...
```{r}[Plots the figures to visualise the results]
fit.coefs |>
  ggplot(aes(x = pred.protein, y = -log10(p.value))) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
